...
class AuthController extends Controller
{
    ...
    function forgotPassword(Request $req)
    {
        ...
        if ($user) {
            $resetPasswordCode = ResetPassword::where("email", $user->email)->first();

            if ($resetPasswordCode !== null) {
                return response()->json([
                    'message' => "Reset password code has been sent to $user->email"
                ], 200);
            } else {
                ...
                Mail::to($user)->send(new ResetPasswordCode($resetPassword->code));
    ...
    function resetPassword(Request $req)
    {
        ...
        $resetPassword = ResetPassword::where("code", $req->code)->first();
        $timestampCarbon = Carbon::parse($resetPassword->created_at);
        $currentTime = Carbon::now();
        $timeDifferenceInMinutes = $timestampCarbon->diffInMinutes($currentTime);

        // Mengecek apakah waktu melebihi 5 menit atau tidak
        if ($timeDifferenceInMinutes > 5) {
            return response()->json([
                'message' => 'Password reset failed, reset code is expired'
            ], 404);
        } else {
