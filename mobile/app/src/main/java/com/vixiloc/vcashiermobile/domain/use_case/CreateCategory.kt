...
class CreateCategory(private val repository: CategoryRepository, private val getToken: GetToken) {
    operator fun invoke(data:CreateUpdateCategoryRequest): Flow<Resource<OnlyResponseMessage>> = flow {
        val token: String = getToken().first()
        try {
            emit(Resource.Loading())
            val response = repository.createCategory(token, data.toCreateUpdateCategoryRequestDto())
